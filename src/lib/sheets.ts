...
export async function fetchAllData(): Promise<AppData> {
  ...
  const yarnPurchases: YarnPurchase[] = cells(purchaseRows)
    .map(r => ({
      ...
      color: cell(r, 4),
      remainingGrams: cell(r, 5),
      fiber: cell(r, 6),
      photoUrls: parsePhotoUrls(cell(r, 7)),
      colorCodes: parseColorCodes(cell(r, 8)),
      price: cell(r, 9),
      quantity: cell(r, 10),
      gramsPerSkein: cell(r, 11),
      yardage: cell(r, 12),
      totalGrams: cell(r, 13),
      totalYardage: cell(r, 14),
      pricePaid: cell(r, 15),
      currency: cell(r, 16),
      source: cell(r, 17),
      status: cell(r, 18),
      pricePerGram: cell(r, 19),
    }))
